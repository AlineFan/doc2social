...
### 绿色科技：组件与 AI 重点标注执行契约

当用户未指定模板、或明确选择 **绿色科技** 时，以下规则是**默认必做项**；用户明确说“不要改写/不要标重点”时才跳过第 2 步。

1. **先保留结构，再处理强调。** `# 一级标题` 必须渲染为绿色科技的 `PART` 编号标题；`##` 必须渲染为编号胶囊小标题。引用、代码块、列表、图片框和文末三连引导卡都沿用内置渲染器，不能为了压缩内容把它们改成普通段落。
2. **AI 自动标重点。** 先通读完整文章，再在**渲染副本**中用 `==重点==` 标出最值得扫读的 3–8 个短语或短句；不要修改用户的原始 Markdown 文件。
   - 优先级：核心结论 → 关键术语/方法名 → 可执行动作 → 有信息密度的数字或限制条件。
   - 保留用户已有的 `==高亮==` 和 `**加粗**`；不要重复包裹，也不要把它们降级或清除。
   - 每个重点应是可独立理解的短语或一句短结论；避免整段、标题、代码、链接 URL、图片说明和纯情绪化形容词。
   - 密度上限：正文每约 250 个汉字最多新增 1 处；短文至少 3 处、长文最多 8 处。重点不足时宁可少标，不凑数。
3. **样式映射不得丢失。** `==...==` 必须输出为绿色科技的黄色扫光 `<mark>`；`**...**` 保持墨绿加粗；行内代码保持绿色浅底；引用保持黑色左边线和浅灰底；图片保持圆角；一级/二级标题继续使用编号复合组件。
4. **出稿前检查。** 确认所有原始段落、标题、图片和代码都仍在；新增高亮只来自用户正文中已有的字词；高亮数量符合上限；复制到公众号后 `<mark>`、加粗和图片仍可见。
